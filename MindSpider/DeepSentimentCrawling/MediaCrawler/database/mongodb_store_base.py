...
class MongoDBConnection:
    ...
    async def _connect(self):
        ...
        try:
            ...
            # 方式1：优先使用URI连接（适用于云服务）
            if mongo_config.get("uri"):
                connection_url = mongo_config["uri"]
                utils.logger.info(f"[MongoDBConnection] Using MongoDB URI connection")
            else:
                # 方式2：使用分开的配置（适用于本地MongoDB）
                ...
                utils.logger.info(f"[MongoDBConnection] Connecting to {host}:{port}")

            self._client = AsyncIOMotorClient(connection_url, serverSelectionTimeoutMS=10000)
            await self._client.server_info()  # 测试连接
            self._db = self._client[db_name]
            utils.logger.info(f"[MongoDBConnection] Successfully connected to MongoDB, using database: {db_name}")
