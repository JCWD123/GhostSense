...
"""
小红书签名算法 Python 兜底实现
从 MediaCrawler 老仓库移植，作为第三优先级方案

使用方法：
    python xhs_sign.py --url "/api/sns/web/v1/search/notes" --a1 "xxx" --b1 "yyy" --xs "zzz" --xt "123456789"
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='小红书签名算法 Python 兜底实现'
    )
    parser.add_argument('--a1', default='', help='Cookie中的a1值')
    parser.add_argument('--b1', default='', help='localStorage中的b1值')
    parser.add_argument('--xs', '--x-s', dest='xs', default='', help='x-s签名值')
    parser.add_argument('--xt', '--x-t', dest='xt', default='', help='x-t时间戳')
    ...
    # 如果没有提供 x_t，生成当前时间戳
    if not args.xt:
        args.xt = str(int(time.time() * 1000))
    
    # 生成签名
    result = sign(args.a1, args.b1, args.xs, args.xt)
